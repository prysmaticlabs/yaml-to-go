  ...
  function parseStruct(depth: number, innerTabs: number, scope: any, omitempty?: any) {
    ...
    if (flatten && depth >= 2) {
      ...
      for (let i in keys) {
        ...
        if (typeof scope[keys[i]] == "string" && scope[keys[i]].startsWith("0x")) {
          const without0x = scope[keys[i]].slice(2,);
          const numBytes = without0x.length/2;

          if (numBytes === 32 || numBytes === 48 || numBytes === 96)
            append(`",ssz:"size=${numBytes}`);
        }
        append('"`\n');
      }
      indent(--tabs);
